...
export class DeviceAdminComponent implements OnInit {
 ...
 getAdhocReqFiles(deviceAdminDtls) {
  ...
  this.configurationService.showRunLogFilesInfo(obj).subscribe((data: any) => {
 ...
 getFiles() {
  ...
  this.configurationService.showConfigFilesInfo(obj).subscribe((data: any) => {
   this.availableFileNames = data;
   console.log(this.availableFileNames);
  });
 }
 ...
 formatDate(dateVal) {
  ...
  return (
   sMonth + "-" + sDay + "-" + sYear + " " + sHour + ":" + sMinute + " " + sAMPM
  );
 }
 ...
 onItem1Change() {
  this.radioSel1 = this.lastRunTime.find(
   (Item) => Item.value === this.RunTimeRadioSelected
  );
  this.radioSelectedString1 = JSON.stringify(this.radioSel1);
 }

 fieldsChange(evnt: any): void {
  if (evnt.target.checked) {
   if (evnt.target.value == "All") {
    $("#divBackup input:checkbox").each(function () {
     this.checked = true;
     if (this.value != "All") this.disabled = true;
    });
   }
  } else {
   if (evnt.target.value == "All") {
 ...
 FilterApply() {
  ...
  let vendor = (<HTMLSelectElement>document.getElementById("devVendor")).value;
  filter.push("commandId=" + vendor);
  ...
  $("#divBackup input:checkbox").each(function () {
   if (this.checked == true) {
    if (this.value == "All") {
     ...
     if (this.value == "Enabled") filter.push("is_enabled=1");
     if (this.value == "Disabled") filter.push("is_enabled=0");
     if (this.value == "Passed") filter.push("is_new=1");
     if (this.value == "Failed") filter.push("is_new=0");
    }
   }
  });
  if (this.radioSelectedString1 != "") {
   console.log(JSON.parse(this.radioSelectedString1).value);
  }

  console.log(filter);
  let filterval = [
   { type: "is_new", name: "1" },
   { type: "is_enabled", name: "1" },
   //  { type: "is_enabled", name: "0" },
  ];
  console.log(filterval);

  let dataval = JSON.parse(JSON.stringify(this.allDevices));
  console.log(dataval);

  let str = "";

  for (let i = 0; i < filterval.length; i++) {
   str = str + "obj." + filterval[i].type + "==" + filterval[i].name;
   if (i < filterval.length - 1) str = str + " && ";
  }

  console.log(str);
  this.allDevices = this.allDevices.filter((obj) => str);
  // this.allDevices = this.allDevices.filter(
  //  (obj) => obj.is_new == 1 && obj.is_enabled == 1
  // );
