...
export class GraphItemsComponent implements OnInit, AfterViewInit {
 ...
 ngAfterViewInit() {
  ...
  var setZoomSlider = new ZoomSlider(this.myDiagram, {
   alignment: new go.Spot(0, 0, 430, 5),
   alignmentFocus: new go.Spot(0, 0, 430, 5),
   size: 180,
   buttonSize: 35,
   orientation: "vertical",
  });

  var appendContextMenu = $(go.HTMLInfo, {
  ...
  function displayContextMenu(obj, diagram, tool) {
   var cmd = diagram.commandHandler;
   console.log("obj", obj);
   console.log("obj.data", obj.data);
   console.log("obj.data.context_menu", obj.data.context_menu);

   var contextMenuDIV = document.getElementById("contextMenuDIV");
   var pt = diagram.lastInput.viewPoint;
   contextMenuDIV.style.left = pt.x + 10 + "px";
   contextMenuDIV.style.top = pt.y + 10 + "px";
   document.getElementById("contextMenuData").innerHTML = obj.data.context_menu;

   document.querySelectorAll(".contextMenuOnclickClass").forEach((element) => {
    element.addEventListener("click", (comp) => {
     contextMenuItemClick(comp);
    });
   });
 ...
 ngAfterViewResonse(resArray) {
  ...
  this.myDiagram.model = go.Model.fromJson({
   ...
   nodeDataArray: resArray[0],
   linkDataArray: resArray[1],
  });
